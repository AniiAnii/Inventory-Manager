...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Debug: Check the extracted values
    echo "Sifra dela: " . $sifra_dela . "<br>";
    echo "Naziv dela: " . $naziv_dela . "<br>";
...
// Check if there are any errors in fetching firms
if (!$result_parts) {
    $errors[] = "Error fetching orders: " . $conn->error;
}
?>
...
<body class="body">
    <div class="container">
        ...
        <form class="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            ...
            <select id="selected_parts" name="sifra_dela" class="input"> <!-- Change name attribute to match the POST variable -->
                <?php
                // Display parts in the dropdown menu
                if ($result_parts->num_rows > 0) {
                    while ($row_parts = $result_parts->fetch_assoc()) { // Change variable name from $row_firm to $row_parts
                        echo "<option value='" . $row_parts["Sifra"] . "|" . $row_parts["Naziv"] . "'>" . $row_parts["Sifra"] . " - " . $row_parts["Naziv"] . "</option>"; // Concatenate Sifra and Naziv with a delimiter (e.g., "|")
                    }
                } else {
